...
local spiralLength = 1
local distance = 1
local INV_SPACE_TO_DUMP = 3
function ChunkBreaker.main()
	...
	while (not redstone.getInput("back")) do
		...
		ChunkBreaker.checkFuel()
		print("current fuel: "..turtle.getFuelLevel().."/"..turtle.getFuelLimit())
		ChunkBreaker.checkInv()

		ChunkBreaker.updateDirection()
...
function ChunkBreaker.updateDirection()
	distance = distance + 1
...
function ChunkBreaker.checkFuel()
...
function ChunkBreaker.checkInv()
	if (#AdvancedTurtle.findInvSpace() < INV_SPACE_TO_DUMP) then
		if (AdvancedTurtle.sortUpInv() <= INV_SPACE_TO_DUMP and (not ChunkBreaker.dumpToBox())) then
			ChunkBreaker.goOrigin(ChunkBreaker.dumpOrigin)
		end
	end
end
...
function ChunkBreaker.dumpToBox()
...
function ChunkBreaker.dumpOrigin()
	-- with out storage
...
function ChunkBreaker.goOrigin(todo)
